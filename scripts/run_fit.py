...
def main() -> None:
    ...
    # NMMA's own CLI declares --xlim with nargs="*" (space-separated:
    # "--xlim -1.00 47.50"), but the archived manifests -- generated by the
    # Astro-COLIBRI pipeline, which never passed --plot -- all use a single
    # comma-joined token instead ("--xlim=-1.00,47.50"). argparse then hands
    # plotting_utils.check_limit() a one-element list (['-1.00,47.50']), not
    # a string, so its `if isinstance(lim, str): lim.split(",")` branch never
    # triggers and it crashes on float('-1.00,47.50'). This bug was always
    # latent in NMMA; it only surfaces now that --plot is actually exercised.
    # Fix on our side: split any comma-joined --xlim/--ylim token into the
    # separate CLI arguments NMMA's parser actually expects.
    for flag in ("--xlim", "--ylim"):
        for i, tok in enumerate(command):
            if tok.startswith(flag + "="):
                value = tok.split("=", 1)[1]
                command[i : i + 1] = [flag, *value.split(",")]
                break
